...
The example generates data from scratch, with 60 taxon, 40 subjects, and 3 covariates. 
...
Here we generate such that rawCount1 is associated with x1, rawCount2 is associated with x2, and the sparsity level is 25%. 
...
If you already have a SummarizedExperiment format data, you can ignore the above steps. Next we analyze the data to test the association between microbiome and the variable `"x1", "x2"` while adjusting for the variables (potential confounders) `"x3"`.
...
sig_results
```
The results found `"rawCount1"` associated with `"x1"`, and `"rawCount2"` associated with `"x2"`, while adjusting for `"x3"`. The regression coefficients and their 95% confidence intervals are provided. These coefficients correspond to $\beta^k$ in the model equation. 

The interpretation is that 

- Every unit increase in `"x1"` is associated with approximately 13.5% increase in the absolute abundance of `"rawCount1"`, every unit increase in `"x2"` is associated with approximately 19.1% increase in the absolute abundance of `"rawCount2"`, in the entire gut ecosystem. 
...
- Every unit increase in `"x1"` is associated with approximately 2.77% increase in the abundance ratio of `"rawCount18"` over `"rawCount34"`  (while controlling for `"x2"` and `"x3"`); Every unit increase in `"x2"` is associated with approximately 2.44% increase in the abundance ratio of `"rawCount18"` over `"rawCount34"` (while controlling for `"x1"` and `"x3"`); Every unit increase in `"x3"` is associated with approximately 252.22% (=exp(1.259)-1) increase in the abundance ratio of `"rawCount18"` over `"rawCount34"` (while controlling for `"x1"` and `"x2"`).

If interested in significant taxon, they could be extracted by `subset(summary_res,sig_ind==TRUE)`. 
